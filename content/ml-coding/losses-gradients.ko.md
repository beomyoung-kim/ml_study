...
## 왜 cross-entropy인가? (CE vs KL, BCE vs CE, 그리고 왜 L1/L2가 아닌가)

### Cross-entropy vs KL divergence
참 분포 $p$와 예측 분포 $q$에 대해:
...
즉 **CE = label의 entropy + KL**입니다. 지도 학습에서는 label $p$가 *고정*이므로 $H(p)$는 상수이고(one-hot label에서는 $H(p)=0$) — 따라서 **CE를 최소화하는 것은 정확히 $D_{\mathrm{KL}}(p\Vert q)$를 최소화하는 것**, 즉 예측과 정답 사이의 "거리"를 줄이는 것입니다. CE를 최적화하는 이유는 이 상수를 떨궈내고 계산이 더 싸기 때문입니다.

<dl class="kv">
<dt>Use CE</dt><dd>표준 분류 loss — target이 고정된(보통 one-hot) label일 때. 정답 클래스의 negative log-likelihood입니다.</dd>
<dt>Use KL</dt><dd><b>양쪽 모두</b>가 실제 분포일 때: <b>knowledge distillation</b>(soft teacher target), <b>VAE</b>(prior로의 KL), <b>RLHF/PPO</b>(policy를 reference 근처에 유지하는 KL penalty), variational inference. KL은 <b>비대칭</b>입니다($D_{\mathrm{KL}}(p\Vert q)\ne D_{\mathrm{KL}}(q\Vert p)$): forward KL은 mode-covering, reverse KL은 mode-seeking.</dd>
</dl>

### BCE vs CE — 기능적으로 다름
- **BCE (출력별 sigmoid):** 각 출력이 **독립적인** yes/no입니다. **multi-label**("cat"과 "outdoor"가 둘 다 참일 수 있음)에 사용 — $C$개의 독립 Bernoulli 문제이며, 확률의 합이 1일 **필요가 없습니다**.
- **CE (클래스에 대한 softmax):** **상호 배타적** 클래스에 대한 하나의 **categorical**이며, softmax가 logit들을 *결합*해서 확률의 합이 **1이 되게** 합니다. single-label multi-class에 사용.
- 2-클래스 경우에는 둘이 일치합니다: **2-way softmax-CE ≡ 하나의 logit에 대한 BCE**. 요령: *상호 배타적 → softmax-CE; 독립/multi-label → BCE.*
...
**2) 확신에 찬 오답에 무한한 penalty.** $p_y\to0$일 때 $-\log p_y\to\infty$이므로, CE는 확신을 갖고 틀린 것을 강하게 벌하고 calibrated된 확률로 밀어붙입니다. MSE의 penalty는 유한합니다($\le1$).
...
**3) 올바른 likelihood / convexity.** CE는 Bernoulli/Categorical 모델 하에서의 negative log-likelihood — 클래스 label에 대한 *올바른* 확률적 loss입니다. MSE는 **Gaussian** likelihood에 대응합니다(regression에는 맞고 category에는 틀림). 또한 CE는 logit에 대해 convex인 반면, sigmoid를 거친 MSE는 non-convex이고 나쁜 local minima를 가집니다. **L1/L2가 *맞는* 경우:** regression / 연속 target, 그리고 bounding-box 좌표 regression(smooth-L1 / Huber).
