...
> [!TIP] 왜 셋업에 공을 들이나
> 2025~2026년 대부분의 RS/AS loop은 여전히 **원격**입니다 (일부 대면 온사이트가 돌아오고 있긴 합니다). 얼어붙은 화면 공유, 죽은 마이크, 어두운 방은 당신의 콘텐츠 우위를 낭비된 시간과 면접관의 짜증으로 바꿔버립니다. 이건 전체 과정에서 가장 값싼 우위입니다. **한 시간의 준비로 로지스틱스를 거의 완벽하게 만들 수 있습니다.** 한 번 제대로 하고 모든 라운드에서 잊어버리세요.

여러 시간대에 걸쳐 면접하는 후보(서울 → 미국/EU)라면, 셋업은 **이상한 시간대와 비원어민 영어 오디오 채널**까지 견뎌야 합니다. 명료함이 평소보다 더 중요합니다.
...
오디오 품질이 비디오보다 더 중요합니다. 또렷한 마이크는 비원어민 억양도 완벽히 알아듣게 유지하지만, 울리는 방의 노트북 마이크는 그러지 못합니다.

| 요소 | 값싼 해결책 | 왜 중요한가 |
| --- | --- | --- |
| **마이크** | 유선 이어버드/헤드셋 마이크 > 노트북 마이크 | 방 울림과 키보드 소음을 없앰, 비원어민 억양에 더 또렷 |
| **카메라 높이** | 렌즈가 **눈높이**에 오도록 노트북을 올림 | 내려다보면 자신감 없어 보임, 눈높이 = 몰입 |
| **조명** | 창을 마주하거나 램프를 *카메라 뒤에* 둠 | 역광은 당신을 실루엣으로 만듦 |
| **프레이밍** | 머리-어깨, 머리 위 여백 조금 | 너무 가까우면 강렬, 너무 멀면 무관심 |
| **아이 컨택** | 화면 속 얼굴이 아니라 **렌즈**를 봄 | 비디오에서는 렌즈 응시가 *곧* 아이 컨택 |

> [!WARNING] 거울이 아니라 실제 도구로 테스트하세요
> Photo Booth에서 "카메라 됨" ≠ "면접 앱에서 됨". 실제로 쓸 **같은 앱, 같은 기기, 같은 네트워크**에서 실제 테스트 콜(친구, 또는 플랫폼 테스트룸)을 하세요. 마이크가 엉뚱한 기기가 아닌지, 화면 공유가 실제로 전송되는지 확인하세요.
...
<dl class="kv">
<dt>CoderPad / Codility / HackerRank</dt><dd>공유 에디터, 종종 미니멀. 연습: IDE autocomplete 없이 작성, 언어 선택기 사용, 허용되면 테스트 실행. Meta 폰 스크린은 보통 CoderPad입니다.</dd>
...
## AI 보조 coding 라운드 (2026)

일부 회사(특히 Meta)는 이제 assistant/autocomplete가 허용되거나 기대되는 **AI 보조 coding 라운드**를 운영합니다. recruiter에게 정책을 확인하고, 허용된다면 *workflow*를 연습하세요: scaffolding을 프롬프트하고, 그다음 비판적으로 읽고, 검증하고, 편집. 시험하는 signal이 "syntax를 기억하는가"에서 "도구를 조종하고 검증하는가"로 이동합니다. 검증을 내레이션하고, 출력을 맹목적으로 받아들이지 마세요.

## 백업 플랜 (무언가는 *반드시* 고장 납니다)
...
- **폰 핫스팟을 미리 설정**해서 네트워크 fallback으로.
