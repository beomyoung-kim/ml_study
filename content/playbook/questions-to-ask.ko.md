...
<dt>출판 정책</dt><dd>"출판 / 오픈소스 정책이 *실제로* 어떤가요?" — 논문 기록이 있는 RS에게 결정적, 특히 Apple(기밀)과 ByteDance에서 날카로움.</dd>
</dl>

## 시니어 IC / bar-raiser — 기준과 장기 관점

"As Appropriate"(Microsoft), bar-raiser(Amazon 스타일), 또는 팀 밖의 staff/principal가 장기 잠재력과 조직 기준을 평가합니다. 기준과 과학에 대해 물으세요.
...
PM, serving/eng, 또는 applied 파트너에게: research와 product의 이음매를 탐침하세요 — 바로 당신의 CV가 강한 역량입니다 (ZIM → CLOVA-X, foreground-API, on-device serving).
...
| "crunch 동안 work-life balance는 어떻게 다루나요?" | "우리 여기서 열심히 해요 😅" / 웃어넘김 | 정상화된 과로 (ByteDance "996" 평판) |
...
각각 조직의 실제이고 공개된 특성을 겨냥합니다 — 그걸 물으면 사전 조사를 했음을 증명합니다 ([Company Playbooks](#/process/companies) 참고).

| 회사 | 물을 것 | 왜 통하는가 |
| --- | --- | --- |
| **Meta / FAIR** | "프로젝트가 FAIR에 있을지 product/GenAI 조직에 있을지 어떻게 결정하나요?" | 조직 분할을 안다는 걸 보여줌, 출판-vs-출시 기대를 명확히 함 |
| **Apple** | "기밀 제약 안에서 scientist가 *무엇을* 출판하거나 오픈소스할 수 있나요?" | product를 캐지 않으면서 문화를 인정함 |
| **NVIDIA** | "research 프로젝트가 최신 하드웨어에 얼마나 기대고, GPU 접근이 병목인가요?" | 그들의 시스템/GPU 정체성에 부합 |
| **Adobe** | "research 결과와 Firefly/Creative-Cloud 기능 사이 loop이 얼마나 촘촘한가요?" | 그들의 research→product 강점을 탐침 |
| **ByteDance Seed** | "research 팀의 OKR은 어떻게 정해지고, 긴 호흡의 bet은 어떻게 보호되나요?" | 빠른/OKR 문화 *와* 깊은 research의 생존 여부를 드러냄 |
| **Mistral** | "scientist 기준 고객 프로젝트 작업과 내부 foundation research의 비율은 어떤가요?" | 그들의 비즈니스 모델을 고려하면 결정에 관련됨 |
| **Microsoft / MSR** | "'As Appropriate' / 조직 간 협업이 실제로 research agenda를 어떻게 형성하나요?" | 그들의 구조를 이해함을 보여줌 |
...
**짧게:** 절대 "없어요"라고 하지 마세요. 공을 그들에게 돌리는 fallback을 쓰세요.
...
**짧게:** 아니요 — 모든 연봉/레벨/로지스틱스는 recruiter로 라우팅하세요.
